...
/** 基础插件 */
export class SeedPlugin<P extends SeedPluginOptions = SeedPluginOptions> {
  /**
   * 插件名称
   * @description
   * 继承时请保证名称不统一
   */
  static PLUGIN_NAME = 'seed-plugin'
  ...
  /**
   * 插件能力使用
   * @param Plugin 必须为继承 `SeedPlugin` 的插件类
   * @description
   ...
   */
  public use<T extends Class<SeedPlugin>>(Plugin: T) {
 ...
 * @description
 * 与 SeedPlugin 相互引用
 * - OutdatedPlugin 继承 SeedPlugin
 * - SeedPlugin 在执行时才会调用 OutdatedPlugin
 */
export class OutdatedPlugin extends SeedPlugin {
  static PLUGIN_NAME = 'outdated-plugin'

  /** 是否已经检测过 */
  protected hasChecked?: boolean

  /** 检查过期 */
  public async checkOutdated() {
    // 检查过就不在检查
    if (this.hasChecked) {
      return
    }
  ...
  public async apply(compiler: Compiler) {
    ...
    compiler.hooks.afterDone.tap(this.pluginName, () => {
      this.checkOutdated().catch((error) => {
